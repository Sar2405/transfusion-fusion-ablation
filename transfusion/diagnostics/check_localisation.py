...
Old-code baseline for comparison:
    full      ~20 m from nearest GT   (systematic displacement)
    cls_only  ~2-9 m from nearest GT  (coarse, near the 4 m threshold)

After the centre-offset fix, confident predictions should sit much closer,
because each query now predicts a small delta from its heatmap-peak position
rather than regressing absolute position from scratch.
...
def main():
    ...
    with torch.no_grad():
        for i in range(min(args.samples, len(ds))):
            ...
            # forward() takes explicit tensors, not a dict
            out = model(
    ...
    if med < 2.0:
        ...
        print("  Comparable to the old cls_only behaviour (2-9 m).")
    else:
        print(f"  DISPLACED — median {med:.2f} m. Predictions are not near GT")
        print("  at all; comparable to the old `full` failure (~20 m).")
    print("=" * 60)
